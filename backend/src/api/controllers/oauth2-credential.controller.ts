...
export class OAuth2CredentialController {
  ...
  async handleCallback(
    ...
    if (result.success) {
      return res.redirect(
        `${frontendUrl}/dashboard/credentials?success=true&credentialId=${result.credentialId}`,
      );
    } else {
      return res.redirect(
        `${frontendUrl}/dashboard/credentials?success=false&error=${encodeURIComponent(result.error || "Unknown error")}`,
      );
    }
  }
