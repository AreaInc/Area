...
describe("ServicesController", () => {
    ...
    describe("getAllServices", () => {
        it("should return services with actions", async () => {
            ...
            actionRegistryMock.getByProvider.mockReturnValue([
                { id: "send-email", name: "Send", description: "Desc", inputSchema: {}, outputSchema: {} }
            ]);
    ...
    describe("getService", () => {
        it("should return service details", async () => {
            servicesServiceMock.getService.mockResolvedValue({ id: 1, provider: "gmail" });
        ...
        it("should throw if service not found", async () => {
            ...
            await expect(controller.getService("gmail")).rejects.toThrow(HttpException);
        });
    });
    ...
    describe("getServiceActions", () => {
        ...
        it("should throw if no actions and no service", async () => {
            actionRegistryMock.getByProvider.mockReturnValue([]);
            servicesServiceMock.getService.mockResolvedValue(null);
            await expect(controller.getServiceActions("gmail")).rejects.toThrow(HttpException);
        });
    });

    describe("getAction", () => {
        it("should return action details", async () => {
            actionRegistryMock.get.mockReturnValue({ id: "send-email", inputSchema: {}, outputSchema: {} });
        ...
        it("should throw if action not found", async () => {
            ...
            expect(() => controller.getAction("gmail", "unknown")).toThrow(HttpException);
        });
    });
});
