...
@Injectable()
export class GoogleCalendarPollingService implements OnModuleInit, OnModuleDestroy {
    ...
    async startPolling() {
        ...
        this.isPolling = true;
        this.logger.log(`Starting Google Calendar polling (interval: ${this.pollIntervalMs}ms)`);
    ...
    private async pollAllRegistrations() {
        ...
        const credentialRows = await this.db.select().from(credentials).where(inArray(credentials.id, Array.from(credentialsToCheck)));
        const credentialsById = new Map(credentialRows.map(c => [c.id, c]));

        await Promise.allSettled(
            Array.from(workflowsByCredential.entries()).map(async ([credentialId, wfSettings]) => {
    ...
    private async checkCredentialEvents(credential: CredentialRow, workflows: WorkflowRegistration[]) {
        ...
        try {
            ...
            const syncToken = this.lastSyncTokens.get(credential.id) || (refreshed.lastHistoryId ? refreshed.lastHistoryId : undefined);
            ...
            try {
                ...
                if (result.events && result.events.length > 0) {
                    for (const event of result.events) {
                        ...
                        if (event.status === 'confirmed' && event.created) {
                            ...
                            if (Date.now() - createdTime < 5 * 60 * 1000) {
                                for (const wf of workflows) {
                                    ...
                                    if (this.newEventTrigger.getRegistrations().has(wf.workflowId)) {
                                        await this.workflowsService.triggerWorkflowExecution(wf.workflowId, {
                                            eventId: event.id,
                        ...
                        if (event.status === 'cancelled') {
                            for (const wf of workflows) {
                                if (this.eventCancelledTrigger.getRegistrations().has(wf.workflowId)) {
                                    await this.workflowsService.triggerWorkflowExecution(wf.workflowId, {
                                        eventId: event.id,
                ...
                if (err.message && (err.message.includes("syncToken") || err.code === 410)) {
                    // Sync token expired/invalid, do a full sync without token to get a new one
                    this.logger.warn(`Sync token expired for credential ${credential.id}, resetting.`);
            ...
            this.logger.error(`Failed to poll Calendar for ${credential.id}: ${error.message}`);
        } finally {
