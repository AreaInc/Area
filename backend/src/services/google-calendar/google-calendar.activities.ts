...
async function ensureFreshAccessToken(credential: {
  ...
  if (
    ...
    throw new Error(
      "Missing OAuth2 refresh credentials, cannot refresh token.",
    );
  }
...
export async function createEventActivity(input: CreateEventInput) {
  ...
  activity.log.info("Creating google calendar event", {
    summary: input.summary,
  });

  try {
    const credential = await ensureFreshAccessToken(
      await loadCredentials(input.credentialId, input.userId),
    );
    const client = getClient(credential);

    const summary = replaceTemplateVariables(input.summary, input.triggerData);
    const description = input.description
      ? replaceTemplateVariables(input.description, input.triggerData)
      : undefined;
    const location = input.location
      ? replaceTemplateVariables(input.location, input.triggerData)
      : undefined;
