...
describe("GoogleCalendarPollingService", () => {
    ...
    beforeEach(async () => {
        ...
        newEventTriggerMock = { getRegistrations: jest.fn().mockReturnValue(new Map()) };
        eventCancelledTriggerMock = { getRegistrations: jest.fn().mockReturnValue(new Map()) };
        ...
        service = module.get<GoogleCalendarPollingService>(GoogleCalendarPollingService);
    });
    ...
    it("should process new events", async () => {
        newEventTriggerMock.getRegistrations.mockReturnValue(new Map([[1, { credentialsId: 10, config: {} }]]));
        ...
        dbMock.from.mockReturnValueOnce({ // credentials
            where: jest.fn().mockResolvedValue([{
                id: 10, userId: "user1", serviceProvider: "google-calendar",
                accessToken: "token",
