...
describe("OAuth2Service", () => {
    ...
    beforeEach(async () => {
        ...
        (google.auth.OAuth2 as unknown as jest.Mock).mockReturnValue(oauth2ClientMock);
    ...
    describe("getAuthUrl", () => {
        ...
        it("should throw if unknown provider", async () => {
            ...
            await expect(service.getAuthUrl("user1", 99)).rejects.toThrow(BadRequestException);
        });
    });
    ...
    describe("handleCallback", () => {
        it("should process google callback", async () => {
            ...
            (service as any).stateStore.set(state, { userId: "user1", credentialId: 1, timestamp: Date.now() });
        ...
        it("should throw if state is invalid", async () => {
            await expect(service.handleCallback("code", "invalid")).rejects.toThrow(UnauthorizedException);
        });
    });
