...
export class GmailPollingService implements OnModuleInit, OnModuleDestroy {
  ...
  private async getRecentEmailsDirect(
    ...
    this.logger.debug(`Found ${messagesList.length} message(s) in inbox`);
    ...
    for (const msgRef of messagesList) {
      ...
      const subject = String(headerMap.get("subject") || "");

      this.logger.debug(
        `Checking email: "${subject}" from "${from}" - X-Area-Generated: ${!!headerMap.get("x-area-generated")}`,
      );

      // Only skip emails generated by Area system
      if (headerMap.get("x-area-generated")) {
        this.logger.debug(`Skipping: X-Area-Generated header present`);
        continue;
      }

      // Allow self-sent emails (user might want to trigger on forwarded emails or test emails)
      ...
      this.logger.debug(`Included email: "${subject}"`);
    }
  ...
  private async getNewEmails(
    ...
    this.logger.debug(
      `Found ${history.length} history entries with ${history.reduce((acc, h) => acc + (h.messagesAdded?.length || 0), 0)} new message(s)`,
    );
    ...
    for (const entry of history) {
      ...
      for (const msg of entry.messagesAdded) {
        ...
        this.logger.debug(
          `Processing history email: "${subject}" from "${from}"`,
        );
        ...
        const details = gmailClient.getMessageDetails(data);
        this.logger.debug(`Including email: "${subject}"`);
        messages.push({
          messageId: details.id,
          threadId: details.threadId,
          from: details.from,
