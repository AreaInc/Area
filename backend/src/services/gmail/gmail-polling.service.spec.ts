...
describe("GmailPollingService", () => {
    ...
    gmailMock = {
      users: {
        getProfile: jest.fn().mockResolvedValue({
          data: { historyId: "100", emailAddress: "me@test.com" },
        }),
        history: {
        ...
        messages: {
          get: jest
            .fn()
            .mockResolvedValue({ data: { payload: { headers: [] } } }),
        },
      },
    };
  ...
  describe("pollAllRegistrations", () => {
    ...
    it("should process registrations", async () => {
      triggerMock.getRegistrations.mockReturnValue(
        new Map([
      ...
      // Mock getting emails
      const checkEmailsSpy = jest.spyOn(
        service as any,
        "checkCredentialEmails",
      );
