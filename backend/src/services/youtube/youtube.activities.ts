...
export interface CreatePlaylistInput { title: string; description?: string; credentialId: number; userId: string; }
export async function createPlaylistActivity(input: CreatePlaylistInput) {
    ...
    return { success: true, playlistId: playlist.id, title: playlist.snippet?.title };
}

export interface DeletePlaylistInput { playlistName: string; credentialId: number; userId: string; }
export async function deletePlaylistActivity(input: DeletePlaylistInput) {
    ...
    if (!playlist || !playlist.id) throw new Error(`Playlist ${input.playlistName} not found`);
    await client.deletePlaylist(playlist.id);
    return { success: true };
}

export interface RateVideoInput { videoId: string; rating: "like" | "dislike" | "none"; credentialId: number; userId: string; }
...
export interface SubscribeChannelInput { channelName: string; credentialId: number; userId: string; }
export async function subscribeChannelActivity(input: SubscribeChannelInput) {
    ...
    if (!channel || !channel.id?.channelId) throw new Error(`Channel ${input.channelName} not found`);
    await client.subscribe(channel.id.channelId);
    return { success: true, channelId: channel.id.channelId };
}

export interface UnsubscribeChannelInput { channelName: string; credentialId: number; userId: string; }
export async function unsubscribeChannelActivity(input: UnsubscribeChannelInput) {
    const client = await getClient(input.credentialId, input.userId);
    const channel = await client.searchChannel(input.channelName);
    if (!channel || !channel.id?.channelId) throw new Error(`Channel ${input.channelName} not found`);
    const subscription = await client.findSubscription(channel.id.channelId);
    if (!subscription || !subscription.id) throw new Error(`Subscription to ${input.channelName} not found`);
    await client.unsubscribe(subscription.id);
    return { success: true };
}

export interface CommentVideoInput { videoId: string; comment: string; credentialId: number; userId: string; }
