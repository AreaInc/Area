...
import { NewLikedVideoTrigger, NewVideoFromChannelTrigger } from "./triggers/youtube-triggers";
...
export class YouTubePollingService implements OnModuleInit, OnModuleDestroy {
    ...
    async startPolling() {
        ...
        this.isPolling = true;
        this.logger.log(`Starting YouTube polling (interval: ${this.pollIntervalMs}ms)`);
        await this.poll();
    ...
    private async checkCredential(credential: any, tasks: any) {
        ...
        try {
            ...
            if (tasks.liked.length > 0) {
                ...
                if (likedVideos.length > 0) {
                    ...
                    if (state.lastLikedVideoId && state.lastLikedVideoId !== latestVideo.id) {
                        ...
                        const data = { videoId: latestVideo.id, title: latestVideo.snippet.title };
                        for (const wid of tasks.liked) await this.workflowsService.triggerWorkflowExecution(wid, data);
                    }
            ...
            if (tasks.channel.length > 0) {
                ...
                for (const wid of tasks.channel) {
                    ...
                    try {
                        ...
                        this.logger.warn(`Failed to check channel ${channelId} for workflow ${wid}`);
                    }
                }
            }
            ...
            this.logger.error(`Error polling YouTube for cred ${credential.id}: ${(e as Error).message}`);
        } finally {
