...
jest.mock("./youtube-client", () => ({
    YouTubeClient: jest.fn().mockImplementation(() => ({
        getLikedVideos: jest.fn().mockResolvedValue([
            { id: "vid1", snippet: { title: "Liked Video" } }
        ]),
...
describe("YouTubePollingService", () => {
    ...
    beforeEach(async () => {
        ...
        likedTriggerMock = { getRegistrations: jest.fn().mockReturnValue(new Map()) };
        channelTriggerMock = { getRegistrations: jest.fn().mockReturnValue(new Map()) };
    ...
    it("should process liked videos", async () => {
        likedTriggerMock.getRegistrations.mockReturnValue(new Map([[1, { credentialsId: 10 }]]));
        ...
        dbMock.from.mockReturnValueOnce({ // credentials
            where: jest.fn().mockResolvedValue([{
                id: 10, userId: "user1", serviceProvider: "youtube",
    ...
    it("should process channel uploads", async () => {
        channelTriggerMock.getRegistrations.mockReturnValue(new Map([[1, { credentialsId: 10, config: { channelId: "ch1" } }]]));
