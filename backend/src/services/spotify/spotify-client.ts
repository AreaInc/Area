...
export class SpotifyClient {
  ...
  constructor(credentials: SpotifyCredentials) {
    ...
    this.clientSecret =
      credentials.clientSecret || process.env.SPOTIFY_CLIENT_SECRET || "";
  }
  ...
  private async ensureValidToken() {
    if (Date.now() >= this.expiresAt - 60000) {
      // Refresh if expiring in less than 1 min
  ...
  async playTrack(trackUri: string) {
    try {
      ...
      if (
        ...
        console.log(
          "No active device found, attempting to find available device...",
        );
        ...
        if (devices.length > 0) {
          ...
          await this.request("PUT", `/me/player/play?device_id=${deviceId}`, {
            uris: [trackUri],
          });
          return;
        } else {
          throw new Error(
            "No Spotify devices available. Please open Spotify on a device to allow playback.",
          );
        }
      }
      throw error;
  ...
  async searchPlaylist(query: string): Promise<any> {
    ...
    const data = await this.request(
      "GET",
      `/search?q=${q}&type=playlist&limit=1`,
    );
