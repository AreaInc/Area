...
export class SpotifyPollingService implements OnModuleInit, OnModuleDestroy {
  ...
  async startPolling() {
    ...
    this.isPolling = true;
    this.logger.log(
      `Starting Spotify polling (interval: ${this.pollIntervalMs}ms)`,
    );
  ...
  private async poll() {
    ...
    const allWorkflowIds = [
      ...trackRegistrations.keys(),
      ...likeRegistrations.keys(),
    ];
    ...
    const addToMap = (wid: number, reg: any, type: "tracks" | "likes") => {
      ...
      if (cred && cred.userId !== wf.userId)
        cred = userDefaultCred.get(wf.userId); // fallback

      if (cred) {
        if (!tasksByCredential.has(cred.id))
          tasksByCredential.set(cred.id, { tracks: [], likes: [] });
  ...
  private async checkCredential(
    credential: any,
    tasks: { tracks: number[]; likes: number[] },
  ) {
    ...
    try {
      ...
      if (tasks.tracks.length > 0) {
        try {
          ...
          this.logger.error(
            `Error polling recently played for cred ${credential.id}: ${(e as Error).message}`,
          );
        }
      }
        ...
        try {
          ...
          if (!state.lastLikedIds || state.lastLikedIds.length === 0) {
            ...
            const newItems = items.filter(
              (item: any) => !savedIds.has(item.track.id),
            );
          ...
          this.logger.error(
            `Error polling liked tracks for cred ${credential.id}: ${(e as Error).message}`,
          );
        }
      }
