...
export class OnActivationTrigger implements ITrigger {
    ...
    async register(workflowId: number, config: Record<string, any>) {
        this.logger.log(`[OnActivationTrigger] Activating workflow ${workflowId} immediately`);
    ...
    private async trigger(workflowId: number) {
        try {
            ...
            this.logger.error(
                `[OnActivationTrigger] Failed to trigger workflow ${workflowId}: ${error instanceof Error ? error.message : String(error)
                }`,
            );
        }
    }
