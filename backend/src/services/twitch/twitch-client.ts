...
export class TwitchClient {
    ...
    constructor(credentials: TwitchCredentials) {
        ...
        this.clientSecret = credentials.clientSecret || process.env.TWITCH_CLIENT_SECRET || "";
    }
    ...
    async getFollowers(broadcasterId: string) {
        const data = await this.request('GET', `/channels/followers?broadcaster_id=${broadcasterId}`);
        return data.data;
    }

    async updateChannelInfo(broadcasterId: string, data: any) {
        await this.request('PATCH', `/channels?broadcaster_id=${broadcasterId}`, data);
    }
    ...
    async createClip(broadcasterId: string, hasDelay: boolean = false) {
        const data = await this.request('POST', `/clips?broadcaster_id=${broadcasterId}&has_delay=${hasDelay}`);
        return data.data[0];
    }
