...
export class StreamStartedTrigger implements ITrigger {
    ...
    outputSchema = { type: "object", properties: { startedAt: { type: "string" }, title: { type: "string" } } };
...
export class StreamEndedTrigger implements ITrigger {
    ...
    configSchema = { type: "object", properties: {} };
    outputSchema = { type: "object", properties: { endedAt: { type: "string" } } };
...
export class NewFollowerTrigger implements ITrigger {
    ...
    configSchema = { type: "object", properties: {} };
    outputSchema = { type: "object", properties: { followerName: { type: "string" }, followerId: { type: "string" }, followedAt: { type: "string" } } };
...
export class ViewerCountThresholdTrigger implements ITrigger {
    ...
    outputSchema = { type: "object", properties: { viewerCount: { type: "number" } } };

    private workflowRegistrations = new Map<number, { config: Record<string, any>; credentialsId?: number }>();
    async register(workflowId: number, config: Record<string, any>, credentialsId?: number): Promise<void> { this.workflowRegistrations.set(workflowId, { config, credentialsId }); }
    async unregister(workflowId: number): Promise<void> { this.workflowRegistrations.delete(workflowId); }
    async validateConfig(config: Record<string, any>): Promise<boolean> { return typeof config.threshold === 'number'; }
    getRegistrations() { return this.workflowRegistrations; }
}
