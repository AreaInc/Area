...
export interface UpdateStreamTitleInput { title: string; credentialId: number; userId: string; }
...
export interface UpdateStreamGameInput { gameName: string; credentialId: number; userId: string; }
...
export interface SendChatMessageInput { message: string; credentialId: number; userId: string; }
...
export interface CreateClipInput { hasDelay?: boolean; credentialId: number; userId: string; }
...
export interface StartCommercialInput { length: number; credentialId: number; userId: string; }
export async function startCommercialActivity(input: StartCommercialInput) {
    ...
    return { success: true, length: result.length, retryAfter: result.retry_after };
}

export interface CreateStreamMarkerInput { description?: string; credentialId: number; userId: string; }
export async function createStreamMarkerActivity(input: CreateStreamMarkerInput) {
    const client = await getClient(input.credentialId, input.userId);
    const user = await client.getUserInfo();
    const marker = await client.createStreamMarker(user.id, input.description || "Marker");
    return { success: true, markerId: marker.id, positionSeconds: marker.position_seconds };
}
