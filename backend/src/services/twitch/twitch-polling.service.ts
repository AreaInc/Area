...
import { StreamStartedTrigger, StreamEndedTrigger, NewFollowerTrigger, ViewerCountThresholdTrigger } from "./triggers/twitch-triggers";
...
export class TwitchPollingService implements OnModuleInit, OnModuleDestroy {
    ...
    async startPolling() {
        ...
        this.isPolling = true;
        this.logger.log(`Starting Twitch polling (interval: ${this.pollIntervalMs}ms)`);
        await this.poll();
    ...
    private async poll() {
        ...
        allRegs.forEach(r => { for (const k of r.keys()) allWorkflowIds.add(k); });
        ...
        const credentialRows = await this.db.select()
            .from(credentials)
            .where(and(eq(credentials.serviceProvider, "twitch"), inArray(credentials.userId, userIds)));
        ...
        const tasksByCredential = new Map<number, { started: number[], ended: number[], follower: number[], viewer: number[] }>();

        const addToMap = (wid: number, reg: any, type: 'started' | 'ended' | 'follower' | 'viewer') => {
            ...
            if (cred) {
                if (!tasksByCredential.has(cred.id)) tasksByCredential.set(cred.id, { started: [], ended: [], follower: [], viewer: [] });
    ...
    private async checkCredential(credential: any, tasks: any) {
        ...
        try {
            ...
            if (tasks.started.length > 0 || tasks.ended.length > 0 || tasks.viewer.length > 0) {
                const streamInfo = await client.getStreamInfo(userId);
                const isLive = !!streamInfo;

                // Stream Started
                if (tasks.started.length > 0 && isLive && !state.isLive) {
                    const data = { startedAt: streamInfo.started_at, title: streamInfo.title };
                    for (const wid of tasks.started) await this.workflowsService.triggerWorkflowExecution(wid, data);
                }

                // Stream Ended
                if (tasks.ended.length > 0 && !isLive && state.isLive) {
                    const data = { endedAt: new Date().toISOString() };
                    for (const wid of tasks.ended) await this.workflowsService.triggerWorkflowExecution(wid, data);
                }
                ...
                if (tasks.viewer.length > 0 && isLive) {
                    ...
                    for (const wid of tasks.viewer) {
                        ...
                        if (reg && viewers >= reg.config.threshold && state.lastViewerCount < reg.config.threshold) {
                            await this.workflowsService.triggerWorkflowExecution(wid, { viewerCount: viewers });
                        }
                    }
                }
            ...
            if (tasks.follower.length > 0) {
                ...
                if (!state.lastFollowers) {
                    ...
                    const newFollowers = followers.filter((f: any) => !savedIds.has(f.user_id));
                    for (const f of newFollowers) {
                        ...
                        for (const wid of tasks.follower) await this.workflowsService.triggerWorkflowExecution(wid, data);
                    }
            ...
            this.logger.error(`Error polling Twitch for cred ${credential.id}: ${(e as Error).message}`);
        } finally {
