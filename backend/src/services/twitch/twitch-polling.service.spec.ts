...
jest.mock("./twitch-client", () => ({
    TwitchClient: jest.fn().mockImplementation(() => ({
        ...
        getFollowers: jest.fn().mockResolvedValue([
            { user_id: "follower1", user_name: "Follower 1", followed_at: new Date().toISOString() }
        ]),
    })),
}));
...
describe("TwitchPollingService", () => {
    ...
    beforeEach(async () => {
        ...
        startedTriggerMock = { getRegistrations: jest.fn().mockReturnValue(new Map()) };
        endedTriggerMock = { getRegistrations: jest.fn().mockReturnValue(new Map()) };
        followerTriggerMock = { getRegistrations: jest.fn().mockReturnValue(new Map()) };
        viewerTriggerMock = { getRegistrations: jest.fn().mockReturnValue(new Map()) };
    ...
    it("should start polling", async () => {
        const pollSpy = jest.spyOn(service as any, "poll").mockResolvedValue(undefined);
        await service.onModuleInit();
        expect(pollSpy).toHaveBeenCalled();
    });

    it("should poll and detect stream started", async () => {
        startedTriggerMock.getRegistrations.mockReturnValue(new Map([[1, { credentialsId: 10 }]]));

        dbMock.from.mockReturnValueOnce({ // workflows
            where: jest.fn().mockResolvedValue([{ id: 1, userId: "user1" }])
        });

        dbMock.from.mockReturnValueOnce({ // credentials
            where: jest.fn().mockResolvedValue([{
                id: 10, userId: "user1", serviceProvider: "twitch",
